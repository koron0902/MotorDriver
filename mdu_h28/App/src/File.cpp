...
std::string FileBase::GetChildrenName()const{
	if (child!=nullptr)return child->GetChildrenNameSub();
	return "";
}
...
string FileInt32::SetData(const std::string& str){
	if(str.empty())return "set: given value is empty"; //NPE prevention
	*data=std::atoi(str.data());
	return "";
}
